...
def remove_epb(rbsp: bytes) -> bytes:
    ...
    while i < len(rbsp):
        ...
        out.append(b); zeros = zeros + 1 if b == 0 else 0; i += 1
    return bytes(out)
...
def parse_sei_user_data_unregistered(rbsp: bytes):
    ...
    while i < n:
        pt = 0
        while i < n and rbsp[i] == 0xFF: pt += 255; i += 1
        if i >= n: break
        pt += rbsp[i]; i += 1
        ps = 0
        while i < n and rbsp[i] == 0xFF: ps += 255; i += 1
        if i >= n: break
        ps += rbsp[i]; i += 1
...
def main():
    ...
    # hp: Annex-B/AU for probing; branch to mp4 gets repacked to AVC
    pipe_str = f"""
